    ...
    public class FakeSwapiProvider : IFakeSwapiProvider
    {
        public async Task<IEnumerable<Starship>> GenerateFakeStarshipsAsync(string? search, CancellationToken ct = default)
        {
            var faker = new Faker<Starship>()
                ...
                .RuleFor(s => s.CostInCredits, f => f.Random.Bool() ? f.Finance.Amount(1000, 1000000).ToString() : "unknown")
                .RuleFor(s => s.Length, f => f.Random.Double(10, 1000).ToString("F2"))
                .RuleFor(s => s.MaxAtmospheringSpeed, f => f.Random.Number(500, 1500).ToString())
                .RuleFor(s => s.Crew, f => f.Random.Number(1, 100).ToString())
                .RuleFor(s => s.Passengers, f => f.Random.Number(0, 500).ToString())
                .RuleFor(s => s.CargoCapacity, f => f.Random.Double(1000, 1000000).ToString("F2"))
                .RuleFor(s => s.Consumables, f => f.PickRandom("1 week", "1 month", "1 year"))
                .RuleFor(s => s.HyperdriveRating, f => f.Random.Double(1, 5).ToString("F1"))
